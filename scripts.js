...
document.addEventListener('DOMContentLoaded', function() {
    // Récupérer le bouton de sauvegarde par son ID
    const saveButton = document.getElementById('save-button');
    if (saveButton) {
        // Ajouter un écouteur d'événement au bouton de sauvegarde
        saveButton.addEventListener('click', function() {
            // Récupérer les données du formulaire
            const formData = new FormData(document.getElementById('diagnostic-form'));
            const data = {};  // Objet pour stocker les données du formulaire
            let pendingFiles = 0;  // Compteur pour les fichiers en attente de conversion

            // Parcourir les entrées du formulaire
            formData.forEach((value, key) => {
                if (value instanceof File && value.size > 0) {
                    // Si la valeur est un fichier, convertir en base64
                    pendingFiles++;
                    getBase64(value, base64 => {
                        data[key] = base64;
                        pendingFiles--;
                        if (pendingFiles === 0) {
                            // Si tous les fichiers sont convertis, sauvegarder les données
                            addFullNameAndSave(data);
                        }
                    });
                } else {
                    // Sinon, ajouter la valeur directement à l'objet data
                    data[key] = value;
                }
            });

            // Si aucun fichier en attente, sauvegarder immédiatement
            if (pendingFiles === 0) {
                addFullNameAndSave(data);
            }
        });
    ...
    // Vérifier si l'élément pour afficher les diagnostics sauvegardés existe
    const savedDiagnosticsElement = document.getElementById('saved-diagnostics');
    if (savedDiagnosticsElement) {
        displaySavedDiagnostics();
    }
...
function getBase64(file, callback) {
    ...
    reader.onload = function () {
        callback(reader.result);  // Appeler le callback avec le résultat en base64
    };
    reader.onerror = function (error) {
        console.error('Error: ', error);  // Afficher une erreur en cas de problème
    };
}
...
>>>>>>> 52de1bc0f2be5d038944b3a6112f87ac53c64c6f
